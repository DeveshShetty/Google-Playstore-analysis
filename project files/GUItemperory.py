...
def screen9():
    ...
    screen_start=tk.Tk()
    screen_start.title("The InterFace")
    screen_start.resizable(False,False) 
    screen_start.geometry('1280x720')

    for i in range(4):
        can=tk.Canvas(screen_start,width=320,height=720,bg='white')#EFEFFB
        canvas.append(can)
    ...
    for i in range(1,4):
        can=tk.Canvas(canvas[i],width=280,height=600,bg='white')#EFEFFB
        canvas.append(can)
        can.place(x=20,y=110)
                
    #labels for interface
    l=tk.Label(canvas[0],text="App'S",bg='White')#EFEFFB
    l.config(font=("Times", 20))
    l.place(x=115,y=50)
    l=tk.Label(canvas[1],text='Positive',bg='white')
    l.config(font=("Times", 20))
    l.place(x=100,y=50)
    l=tk.Label(canvas[2],text='Neutral',bg='white')#EFEFFB
    l.config(font=("Times", 20))
    l.place(x=100,y=50)
    l=tk.Label(canvas[3],text='Negative',bg='White')#EFEFFB
    l.config(font=("Times", 20))
    l.place(x=100,y=50)
    ...
    btn_submit=tk.Button(canvas[0],text='Select',height=5,width=15,fg="white",bg='Black',command=lambda:list_return(canvas,combo,positive,negative,neutral,scroll1,scroll2,scroll3)).place(x=90,y=400)
    btn_submit2=tk.Button(canvas[0],text='Back',height=5,width=15,fg="white",bg='Black',command=screen_start.destroy).place(x=90,y=600)#lambda:list_return(canvas,combo,positive,negative,neutral,scroll1,scroll2,scroll3)
def check(x):
            ...
            if(isinstance(float(x[3].get()), float) and isinstance(float(x[4].get()), float)):
                if(float(x[3].get())<=1 and float(x[3].get())>=-1):
                    ...
                    if(float(x[4].get())<=1 and float(x[4].get())>=-1):
                        ...
                        tsmg.messagebox.showwarning('Wrong Value','Please provide value in range -1 to 1')
                        return True
                else:
...
def fig2():
    ...
    Button(fig2, text="Back", bg="Black", width=10, height=1,font=("Open Sans",13, 'bold'), fg='white',command=fig2.destroy).place(x=1150,y=600)
...
def fig55():
    fig5=Toplevel(screen3)
    fig5.title("QUESTION 5")
    createWindow(fig5)
    Label(fig5,text="What is the download trend category wise over the period for which the data is being made available ?",width="110",height="3",font=Label1_font,fg='white',bg='Black').place(x=0,y=0)
...
#Question 6    
def fig6():
    fig6=Toplevel(screen3)
    fig6.title("QUESTION 6")
    adjustWindow(fig6)
    Label(fig6,text="Yearly Analysis",height="3",font=Label1_font,fg='white',bg='Black').pack(fill=X)
    render=PhotoImage(file="do12.png")#ImageTK.
    img=Label(fig6,image=render)
    img.image=render
    img.place(x=75,y=78)
    Button(fig6, text="Back",bg="Black", width="10",height="1",font=Button1_font,fg='white',command=fig6.destroy).place(x=1200,y=600) 
    OPTIONS = ['2016','2017','2018']
    variable = StringVar(fig6)
    variable.set('CATEGORY')
    w = OptionMenu(fig6, variable, *OPTIONS)
    w.place(x=250, y=220)
    w.configure(bg="Red", fg='white', height="1", font=Button1_font,borderwidth=7,relief=SUNKEN)
    b = Button(fig6, text='SHOW',bg="Grey", width="10", height="1", font=Button1_font, fg='white',borderwidth=7,relief=SUNKEN, command = lambda:figure6(a_dict,variable.get()))
    b.place(x=550, y=220)
    b1 = Button(fig6,text='2016',bg="Grey",width = '10',height = '1',font=Button1_font, fg='white',borderwidth=7,relief=SUNKEN,command = lambda:year_2016(a_dict,2016))
    b1.place(x=250,y=300)
    b1 = Button(fig6,text='2017',bg="Grey",width = '10',height = '1',font=Button1_font, fg='white',borderwidth=7,relief=SUNKEN,command = lambda:year_2017(a_dict,2017))
    b1.place(x=250,y=400)
    b1 = Button(fig6,text='2018',bg="Grey",width = '10',height = '1',font=Button1_font, fg='white',borderwidth=7,relief=SUNKEN,command = lambda:year_2018(a_dict,2018))
    b1.place(x=250,y=500)
    #fig6.mainloop()
    
def fig8():
    
    fig8=Toplevel(screen6)
    fig8.title("QUESTION 8")
    adjustWindow(fig8)
    Label(fig8,text="Question 8",height="2",font=Label1_font,fg='white',bg='Black').pack(fill=X)
     
    # Now starting with the drop down list
    render=PhotoImage(file="d23.png")#ImageTK.
    img=Label(fig8,image=render)
    img.image=render
    #img.place(x=0,y=40)
    img.place(x=75,y=55)
    OPTIONS = ['SPORTS','ENTERTAINMENT','SOCIAL','NEWS_AND_MAGAZINES','EVENTS','TRAVEL_AND_LOCAL','GAME']
    variable = StringVar(fig8)
    variable.set('Select Ypur Choice')
    w = OptionMenu(fig8, variable, *OPTIONS)
    w.place(x=250, y=220)
    w.configure(bg="Black", fg='white', height="1", font=Button1_font)
    b = Button(fig8 , text = 'SHOW',bg="Black", width="10", height="1", font=Button1_font, fg='white',command = lambda:new_plot(TrendDict,variable.get()))
    b.place(x=550, y=220)          
    b1= Button(fig8, text="BACK",bg="Black", width="10", height="1", font=Button1_font,fg='white',command=fig8.destroy).place(x=1200,y=600)
    Label(fig8,text="PYT",height="2",font=Label1_font,fg='white',bg='Black',anchor="Bottom").pack(side=BOTTOM,anchor="sw",fill=X)
    #fig8.mainloop()
    
    
def fig9():
    #TODO: Add Comments 
    fig9=Toplevel(screen4)
    ...
    b1= Button(fig9, text="BACK",bg="black", width="10", height="1", font=Button1_font,fg='white',command=fig9.destroy).place(x=1200,y=600) 
    figure9 = Figure(figsize =(7,7),dpi=85)
    ax9 = figure9.add_subplot(111)
    bar2 = FigureCanvasTkAgg(figure9, fig9)
    bar2.get_tk_widget().pack()#.place(x=70, y=100)
...
def s15():
     Label(text="",height=5,bg="White").pack(fill=X)
...
# Label(screen1, text="", bg='#174873', width='200', height='50').place(x=0, y=120)
 ...
 Button(screen2, text="Click Here", bg="Black", width=10, height=1,font=("Open Sans",13, 'bold'), fg='white',command=fig121).place(x=1150,y=145)#command=first_option
...
def first_screen():
    ...
    img.image=render
    img.place(x=190,y=108)
    #Label(screenn,text="Google Playstore App Launch Study",width="100",height="2",font=("Calibri",22,'bold'),fg='white',bg='#1B4F72').pack()
    Label(screenn,text="Please Enter Your UserName & Password",width=65, font=("Open Sans", 15, 'bold'),fg="black",bg='#7FFF00').pack(pady=10,padx=10)
    ...
    label2=Label(screenn,text="The Python Thugs",bg="#32CD32",fg="White",font="Opensans 19 bold",padx=19,pady=19,borderwidth=3,relief="groove")
    label2.pack(side=BOTTOM,anchor="sw",fill=X) 
